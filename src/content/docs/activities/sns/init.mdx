...
2. **Activer le cache DNS**
   ...
   Cochez l'option **« Activer le cache de requête DNS »**.

3. **Définir les clients autorisés**

   Dans la **« Liste des clients DNS autorisés à utiliser le cache »**, ajoutez les machines, réseaux ou groupes qui pourront bénéficier du service :
   - Cliquez sur **Ajouter**.
...
:::note
Dans le cadre des labs de formation CSNA, le serveur DNS présent sur la DMZ (par exemple `172.16.x.10`) doit être ajouté dans la liste des clients autorisés à utiliser le cache DNS.
:::
...
2. **Dupliquer une politique existante**
   ...
   - Sélectionnez une politique de référence (par exemple la **politique 10** qui contient souvent des règles de base bien structurées).
   - Cliquez-droit sur la politique et sélectionnez **Copier**.
   - Collez-la dans un emplacement libre (par exemple **politique 5**).
...
:::caution
Ne modifiez jamais directement la politique 00 (Default policy) ni les politiques de référence fournies par Stormshield. Travaillez toujours sur une copie pour pouvoir revenir facilement à une configuration fonctionnelle en cas d'erreur.
:::
